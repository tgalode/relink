...
- [x] **Step 1: Documenter la disposition du bloc d'échange**
...
- [x] **Step 2: Documenter le jeu de caractères**
...
- [x] **Step 3: Confronter le plan aux sources**
...
- [x] **Step 1: Ajouter proptest en dépendance de développement**
...
**Files:**
- Already produced (tâche 1) : `docs/protocol/gen1-species-index.md` — ne
  pas le recréer, le lire.
...
- [x] **Step 1: Relire la table déjà sourcée**

La correspondance index interne → numéro national pour les 151 espèces,
plus la liste des index inutilisés (emplacements « MissingNo. »), a déjà
été sourcée et écrite en tâche 1, dans `docs/protocol/gen1-species-index.md`
(format imposé par `docs/protocol/README.md`). Cette étape consiste à la
relire avant de coder dessus, pas à la recréer — la retranscrire changerait
sa source de vérité sans raison et risquerait d'introduire une nouvelle
divergence là où la tâche 1 en a déjà corrigé une (index `0x15`/`0x99`,
voir le document).
...
```bash
git add crates/protocol/src/gen1/ crates/protocol/tests/gen1_species.rs
...
Note : `SpeciesTooRecent` est inatteignable tant que la table de la tâche 5 ne contient que des espèces de première génération. La variante existe pour rendre l'énumération **totale** vis-à-vis des deux règles d'espèce documentées — espèce inconnue, espèce hors Pokédex Gen 1 — plutôt que de dépendre du contenu d'une table. La tâche 7 vérifie qu'elle ne peut pas se déclencher à tort.

> **Corrigé après relecture finale.** Ce paragraphe justifiait initialement la variante par le fait que le codec Gen 2 alimenterait cette même fonction. C'est faux : `eligible_for_gen1` prend un `&gen1::PartyPokemon` et interroge la table d'index interne Gen 1 ; un Pokémon Gen 2 ne peut pas l'alimenter sans changement de signature. Le plan Gen 2 ne doit pas hériter de cette fausse promesse d'API.
...
- [x] **Step 1: Écrire les tests de propriété**
...
- [x] **Step 3: Vérifier que le crate tient réellement sans `std` ni allocateur**
